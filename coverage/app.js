...
const coverageNorth=new THREE.Vector3(0,0,1);
...
function capGeometry(angle){
  const positions=[],rings=8,segments=48,radius=1.009;
  const point=(theta,phi)=>[radius*Math.sin(theta)*Math.cos(phi),radius*Math.sin(theta)*Math.sin(phi),radius*Math.cos(theta)];
...
function updateFootprint(mesh,position){
  const angle=visibilityCentralAngle(Math.hypot(...position),current.minimumElevation),quantizedAngle=Math.round(angle/.002)*.002;
  if(mesh.userData.coverageAngle!==quantizedAngle){mesh.geometry.dispose();mesh.geometry=capGeometry(quantizedAngle);mesh.userData.coverageAngle=quantizedAngle}
  if(!mesh.userData.direction)mesh.userData.direction=new THREE.Vector3();
  mesh.userData.direction.set(...position).normalize();mesh.quaternion.setFromUnitVectors(coverageNorth,mesh.userData.direction);
}
function updateContactLine(line,start,end){
  if(!end){line.visible=false;return}
  line.visible=true;const positions=line.geometry.attributes.position;positions.setXYZ(0,...start);positions.setXYZ(1,...end);positions.needsUpdate=true;
}
...
function renderDynamic(now){
  ...
  let access={elevation:-Math.PI/2,index:0,position:positions[0],visibleSatellites:[]};
  positions.forEach((position,index)=>{const elevation=elevationAngle(position,stationDirection),satellite={elevation,index,position};if(elevation>=current.minimumElevation)access.visibleSatellites.push(satellite);if(elevation>access.elevation)access={...access,...satellite}});access.visible=access.visibleSatellites.length>0;
