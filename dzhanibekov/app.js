...
import { boxDimensionsFromMoments, eulerCoefficients, torqueFreeDerivative } from '../js/rigid.js';
...
let params,omega,q,time=0,playing=true,trail=[],last=performance.now(),flips=0,lastSign=1,solverGeneration=0,equationVersion=0;
...
function updateLiveEquation(){
 ...
 const typeset=()=>{if(version!==equationVersion)return;if(window.MathJax?.typesetPromise){window.MathJax.typesetClear?.([element]);window.MathJax.typesetPromise([element]).catch(()=>{});}else setTimeout(typeset,100)};typeset();
}
function updateSolverReadouts(){
...
function animate(now){requestAnimationFrame(animate);const wall=Math.min(.04,(now-last)/1000);last=now;if(playing&&params){const total=wall*params.speed,sub=Math.max(1,Math.ceil(total/.005));for(let i=0;i<sub;i++)step(total/sub)}if(params){const Lbody=new THREE.Vector3(params.ix*omega[0],params.iy*omega[1],params.iz*omega[2]),Lworld=Lbody.clone().applyQuaternion(q);momentumArrow.setDirection(Lworld.normalize());document.querySelector('#energy').textContent=(.5*(params.ix*omega[0]**2+params.iy*omega[1]**2+params.iz*omega[2]**2)).toFixed(4);document.querySelector('#momentum').textContent=Lbody.length().toFixed(4);document.querySelector('#elapsed').textContent=`${time.toFixed(1)} s`;document.querySelector('#flips').textContent=flips;updateSolverReadouts()}view.render()}requestAnimationFrame(animate);
