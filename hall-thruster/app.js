...
function drawAzimuthalPotential(phase,width,height,ratio){
  const left=.035*width,right=.34*width,top=.835*height,bottom=.955*height,level=mix(top,bottom,.48);ctx.fillStyle='rgba(5,12,15,.82)';ctx.fillRect(left-8*ratio,top-18*ratio,right-left+16*ratio,bottom-top+27*ratio);ctx.strokeStyle='#567078';ctx.lineWidth=1*ratio;ctx.beginPath();ctx.moveTo(left,top);ctx.lineTo(left,bottom);ctx.lineTo(right,bottom);ctx.stroke();ctx.strokeStyle=colors.field;ctx.lineWidth=1.8*ratio;ctx.beginPath();ctx.moveTo(left,level);ctx.lineTo(right,level);ctx.stroke();ctx.fillStyle='#dce9e8';ctx.font=`600 ${9*ratio}px ui-monospace,monospace`;ctx.fillText('φ(θ) at fixed axial z',left,top-6*ratio);ctx.fillText('0',left-2*ratio,bottom+12*ratio);ctx.fillText('2π',right-10*ratio,bottom+12*ratio);ctx.fillStyle=colors.field;ctx.fillText('∂φ/∂θ ≈ 0',left+6*ratio,level-7*ratio);if(phase.stage==='hall'){const theta=((phase.hallAngle%(2*Math.PI))+2*Math.PI)%(2*Math.PI),x=mix(left,right,theta/(2*Math.PI));ctx.fillStyle=colors.electron;ctx.beginPath();ctx.arc(x,level,4.5*ratio,0,Math.PI*2);ctx.fill()}
}
...
function draw(){
  ...
  ctx.fillStyle='rgba(255,177,78,.10)';ctx.fillRect(.25*width,.29*height,.24*width,.14*height);ctx.fillRect(.25*width,.57*height,.24*width,.14*height);label('ionization zone',[.31,.41],project,ratio,colors.ion);
  ctx.fillStyle='rgba(255,118,109,.09)';ctx.fillRect(.47*width,.29*height,.19*width,.14*height);ctx.fillRect(.47*width,.57*height,.19*width,.14*height);label('stronger E / acceleration',[.48,.69],project,ratio,colors.field);
  ...
  label('⊙ azimuthal Hall drift',[.53,.38],project,ratio,colors.electron);label('⊗ azimuthal Hall drift',[.53,.64],project,ratio,colors.electron);
  ...
  if(phase.stage==='hall')label('electron guiding center at this axial station →',[.43,.36],project,ratio,colors.electron);
  ...
  drawAzimuthalPotential(phase,width,height,ratio);label('outward radial B on both sides of centerline',[.40,.92],project,ratio,colors.electron);label('ion beam →',[.84,.31],project,ratio,colors.ion);
}
